...
describe('FLAG_BADGE_MAP', () => {
	it('has entries for all four status values', () => {
		expect(FLAG_BADGE_MAP).toHaveProperty('flag');
		expect(FLAG_BADGE_MAP).toHaveProperty('no_flag');
		expect(FLAG_BADGE_MAP).toHaveProperty('insufficient_evidence');
		expect(FLAG_BADGE_MAP).toHaveProperty('no_data');
	});

	it('each entry has label, colorVar, tintVar, badgeStyle', () => {
		for (const entry of Object.values(FLAG_BADGE_MAP)) {
...
describe('getFlagBadge', () => {
	it('returns the badge for known keys', () => {
		expect(getFlagBadge('flag')).toBe(FLAG_BADGE_MAP.flag);
		expect(getFlagBadge('no_data')).toBe(FLAG_BADGE_MAP.no_data);
	});

	it('returns undefined for unknown keys', () => {
		expect(getFlagBadge('unknown')).toBeUndefined();
	});
});
